...
foreach ($cluster in $clusters){
	foreach ($esxhost in (Get-VMHost -Location $cluster)){
		...
		$newShellTimeOut = $currentShellTimeOut | Set-AdvancedSetting -Value $shellTimeOutValue -confirm:$conrimActions
...
Disconnect-VIserver $vcenter -confirm:$conrimActions